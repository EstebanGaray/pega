...
echo "# GENERATED ON: 2023-01-19T16:33:31+00:00"
echo "#"
...
if [ -z "$INSTALL_FOLDER" ]; then # if INSTALL_FOLDER is not already defined
  read -p "Please enter the installation folder (default: `pwd`/aggregator_8545215f): " INSTALL_FOLDER

  if [ -z "$INSTALL_FOLDER" ]; then
    INSTALL_FOLDER="`pwd`/aggregator_8545215f"
  fi
fi
...
if [ -z "${http_proxy:+w}" ]; then
  ...
  java -jar cht_aggregator.jar setup --endpoint="https://api.cloudhealthtech.com" --token="8545215f-2981-4a6b-b1d5-ebb91d6f848f" --output=cht_aggregator
else
  ...
  debug "Proxy user '${PROXY_USER}'"
  debug "Proxy pass '${PROXY_PASS}'"
  debug java -jar cht_aggregator.jar setup --endpoint="https://api.cloudhealthtech.com" --token="8545215f-2981-4a6b-b1d5-ebb91d6f848f" --output=cht_aggregator --proxy_host="${PROXY_HOST}" --proxy_port="${PROXY_PORT}" --proxy_user="${PROXY_USER}" --proxy_pass="${PROXY_PASS}"
  java -jar cht_aggregator.jar setup --endpoint="https://api.cloudhealthtech.com" --token="8545215f-2981-4a6b-b1d5-ebb91d6f848f" --output=cht_aggregator --proxy_host="${PROXY_HOST}" --proxy_port="${PROXY_PORT}" --proxy_user="${PROXY_USER}" --proxy_pass="${PROXY_PASS}"
fi
